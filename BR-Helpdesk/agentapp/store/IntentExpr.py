...
class IntentExtractor(object): 
    ...
    def startTrainingProcess(self, cust_id):
        ...
        self.model = Word2Vec(self.X, size=10, window=5, min_count=1, workers=2)
        ...
        '''self.etree_w2v_tfidf = Pipeline([("word2vec vectorizer", MeanEmbeddingVectorizer(w2v)), 
                        ("extra trees", Random(n_estimators=200))])
        elf.etree_w2v_tfidf = Pipeline([("word2vec vectorizer", TfidfEmbeddingVectorizer(w2v)), 
                        ("MultinomialNB", MultinomialNB())])'''
        self.etree_w2v_tfidf = Pipeline([("word2vec vectorizer", MeanEmbeddingVectorizer(w2v)), 
                        ("SVC", SVC(kernel='linear', probability=True))]) 
        self.etree_w2v_tfidf.fit(self.X, self.y)
        print ('Post Fitting')
        #pickle_out = pickle.dumps(self.etree_w2v_tfidf, protocol=4)
        from sklearn.externals import joblib
        filename = 'finalized_model.pkl'
        joblib.dump(self.etree_w2v_tfidf, filename)
        print ('Post Pickel')
        import os
        print ('Size of File : ', os.path.getsize(filename))
        tickets_learn = tickets_learner()
        tickets_learn.put_bucket_file(filename, cust_id)                 
        print ("Total Training Samples : %s" % len(self.y))
        print ("startTrainingProcess : Completed " + str(cust_id))
        return
